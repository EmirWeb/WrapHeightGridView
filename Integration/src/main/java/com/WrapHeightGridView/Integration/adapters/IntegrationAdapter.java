...
public class IntegrationAdapter extends BaseAdapter {
    ...
    public View getView(final int position, final View view, final ViewGroup viewGroup) {
        ...
        switch (types) {
            case button:
                final Button button = (Button) getView(view, context, R.layout.list_item_button);
                button.setText(Integer.toString(position));
                return button;
            case imageView:
                final ImageView imageView = (ImageView) getView(view, context, R.layout.list_item_image_view);
                return imageView;
            case textView:
            default:
                final TextView textView = (TextView) getView(view, context, R.layout.list_item_text_view);
